...
import { FaSearch, FaChevronLeft, FaChevronRight, FaStar, FaSpinner } from 'react-icons/fa';
import api from '../api';
...
const MyPerformance = () => {
  ...
  const fetchMyEvaluations = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await api.get('/performance-evaluation/my-evaluations');
  ...
  const filteredEvaluations = evaluations.filter(e =>
    ...
    e.feedback?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading your performance evaluations...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        <div className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-3">
          ...
          <input
            type="text"
            placeholder="Search by evaluation period or feedback..."
            value={searchTerm}
            onChange={(e) => {
            ...
            <tbody>
              {paginatedData.length > 0 ? (
                paginatedData.map((evaluation, idx) => (
                  <tr key={evaluation._id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
