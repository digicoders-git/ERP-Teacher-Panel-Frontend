...
const DashboardContent = ({ setActiveTab }) => {
  ...
  const dashboardCards = [
    ...
    },
    {
      ...
      title: 'E-Diary',
      icon: FaBook,
      count: stats?.stats?.totalStudents ?? 0,
      color: 'bg-teal-500',
    ...
    },
    {
      ...
      title: 'Notices',
      icon: FaBell,
      count: stats?.recentNotices?.length ?? 0,
      color: 'bg-red-500',
      description: 'Recent'
    }
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-indigo-500 text-white p-6 rounded-lg mt-9 shadow-2xl">
        <h1 className="text-2xl font-bold mb-2">Welcome back, {currentTeacher?.name || 'Teacher'}!</h1>
        <p className="text-indigo-100">Subject: {currentTeacher?.subjects?.join(', ') || 'N/A'}</p>
      </div>
            ...
            <div className="space-y-4">
              {stats.upcomingClasses.map((cls) => (
                <div key={cls.id} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-800">{cls.class} {cls.section}</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
              ...
              <p className="text-2xl font-bold text-gray-900">{stats?.todayAttendance?.total ?? 0}</p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
